...
import (
	...
	"snippetbox.fayokunmiosho.com/internal/models/mocks"
)
...
func (ts *testServer) postForm(t *testing.T, urlPath string, form url.Values) (int,
	http.Header, string) {
	...
	// nosurf performs a same-origin Referer check on HTTPS requests.
	// Go's http.Client doesn't set Referer automatically (only browsers
	// do this), so we set it manually here to satisfy that check.
